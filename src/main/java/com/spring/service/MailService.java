...
public class MailService implements MailServiceInter {
	 ...
	 @Autowired
	    ...
	    public MimeMessage creatMessage(String to) throws MessagingException, UnsupportedEncodingException {
	        ...
	        message.setSubject("[Runners High] 회원가입을 위한 이메일 인증코드 입니다"); // 이메일 제목

	        String msgg = "";
	        // msgg += "<img src=../resources/static/image/emailheader.jpg />"; // header image
	        msgg += "<h1>안녕하세요</h1>";
	        msgg += "<h1>달리기 챌린지 플랫폼 Runners High 입니다</h1>";
	        msgg += "<br>";
	        ...
	        message.setFrom(new InternetAddress("ahtj1234@naver.com", "Runners High"));
