...
<!-- <script src="jquery.cookie.js"></script>   -->	
	...
	<div class="site-wrap-width">
		...
		<div class="site-section">
			...
			<div class="context-form">
	        	<label for="region">진행 지역</label><br>
					<div class="selectBox2">
					    <button type="button" class="label" id="region_state" name="region_state">도</button>
					    <input type="hidden" id="region_state_input" name="region_state" value="">
					    <ul class="optionList">
					        <c:forEach var="state" items="${stateList}">
					            <li class="optionItem" value="${state}">${state}</li>
					        </c:forEach>
					    </ul>
					</div><br>
	        		<div class="selectBox2">
					    <button type="button" class="label" id="region_city" name="region_city">시</button>
					    <input type="hidden" id="region_city_input" name="region_city" value="">
					    <ul class="optionList">
					    </ul>
					</div><br>
		        		<div class="selectBox2">
						    <button type="button" class="label" id="region_district" name="region_district">군/구</button>
						    <input type="hidden" id="region_district_input" name="region_district" value="">
						    <ul class="optionList">
						    </ul>
						</div>
...
const allSizeCheck = RegExp(/^[0-9]+$/);	
	...
	$("label[for='all_auth_check']").text("*챌린지 완수 기준(최소 40% ~ 최대 100%)");
	...
	$("label[for='size_check']").text("*모집할 최대 인원 수(최소 3명 ~ 최대 100명)");
...
<!-- 도에 따라 시/군/구 정보 바꾸기 -->
function chageState() {
    var state = $("#region_state_input").val();
    $.ajax({
        async: false,
        type: 'POST',
        ...
        contentType: "application/json; charset=UTF-8",
        success: function (data) {
            $("#region_city ul.optionList").empty(); // 도시 목록 초기화
            $("#region_district ul.optionList").empty(); // 군/구 목록 초기화
            for (var count = 0; count < data.length; count++) {
                $('#region_city ul.optionList').append("<li class='optionItem' data-value='" + data[count] + "'>" + data[count] + "</li>");
            }
        },
        error: function (error) {
            alert("도를 입력해주세요!");
    ...
    // 도를 선택했을 때 군/구를 초기화
    $("#region_district ul.optionList").empty();
    $('#region_district').val("").change();
}

function chageCity() {
    var city = $("#region_city_input").val();
    $.ajax({
        async: false,
        type: 'POST',
        data: JSON.stringify({ value: city }),
        url: "/getDistrict",
        dataType: "json",
        contentType: "application/json; charset=UTF-8",
        success: function (data) {
            $("#region_district ul.optionList").empty();
            for (var count = 0; count < data.length; count++) {
                $('#region_district ul.optionList').append("<li class='optionItem' data-value='" + data[count] + "'>" + data[count] + "</li>");
            }
        },
        error: function (error) {
            alert("시를 입력해주세요!");
...
function chageDistrict() {
    var district = $("select[name=region_district] option.selected").text();
    console.log("선택된 " + district);
}

function submit2(){
	Form=document.regist_form;
	if(Form.chall_start_date.value == "" ||
            Form.chall_end_date.value == "" ||
            Form.chall_all_auth.value == "" ||
            Form.region_district.value == "" ||
            !all_auth_chk ||
            !size_chk){
