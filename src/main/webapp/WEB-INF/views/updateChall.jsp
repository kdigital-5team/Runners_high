...
<form name= "update_form" id="update_form" method="POST" action="/challenge/updateChall${chall.chall_id}">
    <div class="container">
    
        <h1>챌린지 수정</h1>
        <div class="form-group">
            <label for="inputChall">챌린지명</label>
            <input type="text" class="form-control" id="chall_name" name="chall_name" placeholder="챌린지명" value="${chall.chall_name }">
            <label for="challName_check"></label>
        </div>
        <div class="form-group">
            <label for="inputIntro">설명</label>
            <input type="text" class="form-control" id="chall_intro" name="chall_intro" placeholder="챌린지 설명" value="${chall.chall_intro }">
             <label for="chall_intro"></label>
        </div>
        <div class="form-group">
            <label for="inputDate">기간</label>
            <label><input type="text" class="form-control" id="chall_start_date" name="chall_start_date" value="${chall.chall_start_date }"  readonly></label>
            <label><input type="text" class="form-control" id="chall_end_date" name="chall_end_date"value="${chall.chall_end_date }" readonly></label>
        ...
        <div class="form-group">
            ...
            <select id="chall_week_auth" name="chall_week_auth">
				<option value ="1" <c:if test="${chall.chall_week_auth eq 1}">selected="selected" </c:if>> 주 1회</option>
				<option value ="2" <c:if test="${chall.chall_week_auth eq 2}">selected="selected" </c:if>> 주 2회 </option>
				<option value ="3" <c:if test="${chall.chall_week_auth eq 3}">selected="selected" </c:if>> 주 3회 </option>
				<option value ="4" <c:if test="${chall.chall_week_auth eq 4}">selected="selected" </c:if>> 주 4회 </option>
				<option value ="5" <c:if test="${chall.chall_week_auth eq 5}">selected="selected" </c:if>> 주 5회</option>
				<option value ="6" <c:if test="${chall.chall_week_auth eq 6}">selected="selected" </c:if>> 주 6회 </option>
				<option value ="7" <c:if test="${chall.chall_week_auth eq 7}">selected="selected" </c:if>> 매일 </option>
			</select >
        </div>
        <div class="form-group">
        	<label for="chall_size">인원</label>
       		<input type="text" class="form-control" id="chall_size" name="chall_size" placeholder="인원 수" value="${chall.chall_size }">
		</div>
        <div class="form-group">
		...
		<div class="form-group">
        	...
        	<select id="chall_sit" name="chall_sit">
				<option value ="모집중" <c:if test="${chall.chall_sit eq '모집중'}">selected="selected" </c:if>> 모집중</option>
				<option value ="모집종료" <c:if test="${chall.chall_sit eq '모집종료'}">selected="selected" </c:if>> 모집 종료 </option>
				<option value ="모집예정" <c:if test="${chall.chall_sit eq '모집예정'}">selected="selected" </c:if>> 모집 예정 </option>
			</select >
		</div>
		<div class="form-group">
        	...
        	<select id="chall_online" name="chall_online">
				<option value ="온라인" <c:if test="${chall.chall_online eq '온라인'}">selected="selected" </c:if>> 온라인 </option>
				<option value ="오프라인" <c:if test="${chall.chall_online eq '오프라인'}">selected="selected" </c:if>> 오프라인  </option>
				<option value ="모두" <c:if test="${chall.chall_online eq '모두'}">selected="selected" </c:if>> 모두 </option>
			</select >
		</div>
		<div class="form-group">
        	<label for="chall_all_auth">챌린지 최소 인증률</label>
       		<input type="text" class="form-control" id="chall_all_auth" name="chall_all_auth" placeholder="최소 40% 이상 숫자로만 입력해주세요!" value="${chall.chall_all_auth }">
		</div>
